...
const BUFFER_DURATION_SECS: usize = 45;
const BUFFER_SIZE: usize = (SAMPLE_RATE as usize) * BUFFER_DURATION_SECS;
...
impl AudioState {
    pub fn new() -> Result<Self, anyhow::Error> {
        ...
        // Downsampling ratio
        // Simple approach: if input is 48000, we want 16000. Ratio = 3.
        // We will just decimate or average. 
        // For simplicity in this vibe-code, we'll do basic resampling.
        ...
        let stream = match config.sample_format() {
            cpal::SampleFormat::F32 => device.build_input_stream(
                &config.into(),
                move |data: &[f32], _: &_| write_input_data(data, &buffer_clone, input_sample_rate),
                err_fn,
                None
            )?,
            cpal::SampleFormat::I16 => device.build_input_stream(
                &config.into(),
                move |data: &[i16], _: &_| write_input_data_i16(data, &buffer_clone, input_sample_rate),
                err_fn,
                None
            )?,
...
fn write_input_data(input: &[f32], buffer: &Arc<Mutex<VecDeque<f32>>>, input_rate: u32) {
...
fn write_input_data_i16(input: &[i16], buffer: &Arc<Mutex<VecDeque<f32>>>, input_rate: u32) {
    // Convert i16 to f32
    let float_input: Vec<f32> = input.iter().map(|&x| x as f32 / i16::MAX as f32).collect();
    write_input_data(&float_input, buffer, input_rate);
}
...
pub fn transcribe_audio(wav_path: String) -> Result<String, String> {
    ...
    if !output.status.success() {
        return Err(format!("Whisper execution failed: {}", String::from_utf8_lossy(&output.stderr)));
    }
